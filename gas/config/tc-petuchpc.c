...
static struct hash_control *register_hash_control;
...
md_begin (void)
{
	...
	register_hash_control = hash_new();
	...
	for (reg = petuchpc_reg_info; reg->name; reg++)
		hash_insert(register_hash_control, reg->name, (char*)reg);
...
static char* parse_reg (char* str, int* r){
	...
	*str = 0;
	...
	reg = (petuchpc_reg_info_t*)hash_find(register_hash_control, start);
...
md_assemble (char* str)
{
	...
	*op_end = 0;
	...
	opcode = (petuchpc_opc_info_t*)hash_find(opcode_hash_control, op_start);
	*op_end = pend;
