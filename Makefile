...
target_alias=petuchpc-elf
target_vendor=unknown
target_os=elf
target=petuchpc-unknown-elf

program_transform_name = s&^&petuchpc-elf-&
...
TOPLEVEL_CONFIGURE_ARGUMENTS=./configure --target=petuchpc-elf

tooldir = ${exec_prefix}/petuchpc-elf
build_tooldir = ${exec_prefix}/petuchpc-elf
...
BUILD_CONFIGARGS =  --cache-file=../config.cache  --program-transform-name='s&^&petuchpc-elf-&' --disable-option-checking --with-build-subdir="$(BUILD_SUBDIR)"
...
HOST_CONFIGARGS =  --cache-file=./config.cache  --with-gnu-as --with-gnu-ld --with-system-zlib  --program-transform-name='s&^&petuchpc-elf-&' --disable-option-checking
...
TARGET_SUBDIR = petuchpc-elf
# This is set by the configure script to the arguments to use when configuring
# directories built for the target.
TARGET_CONFIGARGS = --cache-file=./config.cache --enable-multilib --with-cross-host=x86_64-unknown-linux-gnu   --program-transform-name='s&^&petuchpc-elf-&' --disable-option-checking --with-target-subdir="$(TARGET_SUBDIR)"
# This is the list of variables to export in the environment when
...
CC_FOR_TARGET=$(STAGE_CC_WRAPPER) petuchpc-elf-cc

# If GCC_FOR_TARGET is not overriden on the command line, then this
# variable is passed down to the gcc Makefile, where it is used to
# build libgcc2.a.  We define it here so that it can itself be
# overridden on the command line.
GCC_FOR_TARGET=$(STAGE_CC_WRAPPER) petuchpc-elf-gcc
CXX_FOR_TARGET=$(STAGE_CC_WRAPPER) petuchpc-elf-c++
RAW_CXX_FOR_TARGET=$(STAGE_CC_WRAPPER) petuchpc-elf-c++
GCJ_FOR_TARGET=$(STAGE_CC_WRAPPER) petuchpc-elf-gcj
GFORTRAN_FOR_TARGET=$(STAGE_CC_WRAPPER) petuchpc-elf-gfortran
GOC_FOR_TARGET=$(STAGE_CC_WRAPPER) petuchpc-elf-gccgo
DLLTOOL_FOR_TARGET=$$r/$(HOST_SUBDIR)/binutils/dlltool
LD_FOR_TARGET=$$r/$(HOST_SUBDIR)/ld/ld-new

LIPO_FOR_TARGET=petuchpc-elf-lipo
